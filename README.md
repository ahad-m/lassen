<p align="center">
  <img src="lasan_logo.png" alt="لَسِن Logo" width="220"/>
</p>

<h1 align="center">لَسِن (Lasan) — Arabic Poetry Web Application</h1>

<p align="center">
  An immersive, AI-powered web experience dedicated to Arabic poetry — exploring classical verses,<br/>
  generating new poetry, interpreting meanings, and journeying through poetic history.
</p>
...
An interactive quiz game where the user is challenged to identify the poet, era, or meter of a displayed verse. When the answer is wrong, the correct answer is retrieved directly from `poems_db.json` and shown to the user.
...
flowchart TD
    ...
    E -->|No| G([System highlights\nthe error in the verse])
    F --> B
    G --> B
...
Three domain-specific models were fine-tuned on Arabic poetry corpora using **AraBERT** and **AraPoemBERT** as base models. All three are loaded into memory at backend startup for low-latency inference.

### Meter Classifier — البحر

| Property | Detail |
|---|---|
| Base model | `arapoembert` |
| Task | Multi-class classification of Arabic prosodic meters (Tawil, Kamil, Basit, Wafir, and others) |
| Accuracy | **98.38%** — Macro F1: **98.39%** (156K samples) |
| Framework | HuggingFace Transformers + PyTorch |

AraPoemBERT is pre-trained specifically on Arabic poetry, making it the strongest base for meter classification. Its poetry-native training corpus allows the fine-tuned model to recognise rhythmic patterns (تفعيلة) with near-perfect accuracy across the traditional 16 Arabic meters.

### Era Classifier — العصر

| Property | Detail |
|---|---|
| Base model | `aubmindlab/arabert` |
| Task | Multi-class classification across Arabic historical periods (Jahili, Islamic, Abbasid, Andalusian, Mamluk, Modern) |
| Accuracy | **83.7%** — Macro Precision: **82.6%**, Macro Recall: **82.7%**, Macro F1: **82.6%** |
| Framework | HuggingFace Transformers + PyTorch |

Fine-tuned over 3 epochs with consistent improvement (Training Loss: 0.506 → 0.358). AraBERT's pre-training on both classical and contemporary Arabic enables the model to detect the lexical and stylistic markers that shift across eras — from the pre-Islamic desert imagery of the Jahili period to the colloquial-influenced diction of modern verse.

### Topic Classifier — الموضوع

| Property | Detail |
|---|---|
| Base model | `aubmindlab/arabert` (v02) |
| Label setup | 7 merged classes |
| Val Accuracy | **60.27%** — Val Macro F1: **54.60%** |
| Test Accuracy | **56.09%** — Test Macro F1: **50.26%** |
| Notes | Final best model — AraBERT v02 outperformed CAMeL-BERT across all setups |
| Framework | HuggingFace Transformers + PyTorch |

The same AraBERT base used for era classification is reused for topic classification. Sharing the base model allows both to be loaded memory-efficiently in the same backend process while independent fine-tuning heads keep per-task accuracy high. Topic classification powers mood-based verse retrieval and the poetry game's thematic challenge mode.
...
- **AraPoemBERT** — fine-tuned meter classifier (98.38% accuracy)
- **AraBERT** (`aubmindlab/arabert`) — fine-tuned era and topic classifiers
- **HuggingFace Transformers + PyTorch** — model loading and inference
- **Tokenizers + Accelerate + Safetensors** — HuggingFace inference stack
- **HuggingFace Hub** — model versioning and hosting
- **NumPy** — embedding vectors and numerical operations
